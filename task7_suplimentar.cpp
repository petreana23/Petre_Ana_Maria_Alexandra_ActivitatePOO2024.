...
public:
    int capacitate;
    string material;
    int lungime; // m?surat? în cm
    bool asezateGen; // indic? dac? c?r?ile sunt a?ezate dup? gen
    string* titluriCarti;
    const int capacitateMaximaRaft; // capacitatea maxim? a raftului
    static int inaltimeStandardRaft; // în?l?imea standard a raftului în cm

    // Constructorul f?r? parametri
    ...
    // Metod? static? pentru în?l?imea standard a raftului
...
class BibliotecaOnline : public Biblioteca {
private:
    string platforma;
    bool accesGlobal;
...
public:
    ...
    BibliotecaOnline(float pret, string platforma, bool accesGlobal) : Biblioteca(pret) {
    ...
    void setPlatforma(string NewPlatforma) {
        platforma = NewPlatforma;
    }

    void setAccesGlobal(bool NewAccesGlobal) {
        accesGlobal = NewAccesGlobal;
    }

    friend ostream& operator<<(ostream& os, BibliotecaOnline& bo) {
        os << "Platforma online: " << bo.platforma << endl;
        os << "Acces global: " << bo.accesGlobal << endl;
        return os;
    }
};
...
public:
    ...
    Abonament(string tipAbonament, float cost, int durataLuna) {
        this->tipAbonament = tipAbonament;
        this->cost = cost;
        this->durataLuna = durataLuna;
    }

    string getTipAbonament() {
        return tipAbonament;
    }
    ...
    void setTipAbonament(string NewTipAbonament) {
        tipAbonament = NewTipAbonament;
    }
    ...
    void setDurataLuna(int NewDurataLuna) {
        durataLuna = NewDurataLuna;
    }

    friend ostream& operator<<(ostream& os, Abonament& ab) {
        os << "Tip abonament: " << ab.tipAbonament << endl;
        os << "Cost: " << ab.cost << " RON" << endl;
        os << "Durata: " << ab.durataLuna << " luna/luni" << endl;
        return os;
    }
};
...
int main() {
    ...
    cititor1 += 1; // Suprasarcin? operator `+=`
    ...
    // Afi?area bibliotecii
    cout << biblioteca1 << endl;
    cout << endl;

    // Verificarea posibilit??ii de împrumut
    if (poateImprumuta(cititor1, r1)) {
        imprumutaCarte(c1, cititor1);
    }
    else {
        cout << cititor1.getNume() << " nu poate împrumuta mai multe c?r?i." << endl;
    }

    // Crearea unui obiect BibliotecaOnline
    BibliotecaOnline bo1(120.50, "E-Library", true);
    cout << bo1 << endl;

    // Crearea unui obiect Abonament
    Abonament abonament1("Premium", 19.99, 6);
    cout << abonament1 << endl;
